...
void keyPressed() {
  if (key == 32) {
    message += key; 
    output = message.substring(4, message.length());
    myPort.write(output);
    delay(100);
    TextToSpeech.say(output, TextToSpeech.voices[7], voiceSpeed);
    println(output);
    fullSentance += output;  
    message = "null";
    output = "";
...
if (key == '\n') {
   delay(100);
   TextToSpeech.say(fullSentance, TextToSpeech.voices[7], voiceSpeed);
   myPort.write(fullSentance);
   println(fullSentance);
   fullSentance = "";
} 
};
